...
const ReviewWidget = ({ pageName = "Accueil", }) => {
        ...
        created(slider) {
            ...
            const forceHeight = () => {
                if (slider.container) {
                    ...
                    const activeSlide = slider.container.querySelector(".keen-slider__slide--active") ||
    ...
    const renderGoogleStars = (rating) => {
        return Array.from({ length: 5 }).map((_, i) => (_jsx("span", { className: `google-star ${i < rating ? "filled" : "empty"}`, "aria-label": `${i + 1} étoile${i > 0 ? "s" : ""}`, onMouseEnter: (e) => e.currentTarget.classList.add("hovered"), onMouseLeave: (e) => e.currentTarget.classList.remove("hovered"), children: "\u2605" }, i)));
    };
    const renderTripAdvisorStars = (rating) => {
        return Array.from({ length: 5 }).map((_, i) => (_jsx("span", { className: `tripadvisor-star ${i < rating ? "filled" : "empty"}`, "aria-label": `${i + 1} étoile${i > 0 ? "s" : ""}`, onMouseEnter: (e) => e.currentTarget.classList.add("hovered"), onMouseLeave: (e) => e.currentTarget.classList.remove("hovered"), children: "\u25CF" }, i)));
    };
